...
if __name__ == '__main__':
    ...
    '''
    item = "1"tInit = time.time()
    recomendador.load_similitudes_item(item)
    similitudes_item = recomendador.desviaciones[item]
    #print(similitudes_item)
    similitudes_item = sorted(similitudes_item.items(), key=operator.itemgetter(1), reverse=True) #retorna lista de tuplas ordenadas por el segundo elemento (rating)
    i = 0
    print("Similitudes del item:", item)
    for item, rating in similitudes_item:
        print("i:", i, " ,item:", item)
        print(rating)
        if i>=10:
            break
    ...
    item = '3'
    t = time.time()
    print(recomendador.predecirSimilitudCosenoAjustado(usuario, item, reload=0))
    print("Time to calculate similitudes:", time.time()-t)
